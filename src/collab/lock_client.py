...
def _resolve_project_root() -> str:
    ...
    """
    override = os.getenv("COLLAB_PROJECT_ROOT")
    if override:
...
def _resolve_runtime_root(project_root: str) -> str:
    """Resolve persistent runtime state directory outside application repos."""
    home_override = os.getenv("COLLAB_HOME")
    if home_override:
        return os.path.abspath(home_override)

    base = os.getenv("LOCALAPPDATA") or tempfile.gettempdir()
    digest = hashlib.sha1(
        project_root.encode("utf-8"), usedforsecurity=False
    ).hexdigest()[:12]
    return os.path.join(base, "collab", "projects", digest)
