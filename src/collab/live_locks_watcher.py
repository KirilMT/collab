...
_PROJECT_ROOT = os.path.abspath(os.getenv("COLLAB_PROJECT_ROOT") or os.getcwd())
_runtime_base = os.getenv("COLLAB_HOME")
if _runtime_base:
    _COLLAB_ROOT = os.path.abspath(_runtime_base)
else:
    _local_app_data = os.getenv("LOCALAPPDATA") or tempfile.gettempdir()
    _proj_hash = hashlib.sha1(
        _PROJECT_ROOT.encode("utf-8"), usedforsecurity=False
    ).hexdigest()[:12]
    _COLLAB_ROOT = os.path.join(_local_app_data, "collab", "projects", _proj_hash)
...
try:
    from .logging_config import setup_collab_logging as _setup_collab_logging_obj
except Exception:
    _setup_collab_logging_obj = None
